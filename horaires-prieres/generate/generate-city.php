...
for ($i=1; $i<2400; $i++)
{
	...
	${'url'.$i} = "../monde/${'continent'.$i}/${'country'.$i}/${'ville'.$i}.html";
 
	if (!file_exists("../monde/${'continent'.$i}")) mkdir("../monde/${'continent'.$i}/", 0700);
	if (!file_exists("../monde/${'continent'.$i}/${'country'.$i}")) mkdir("../monde/${'continent'.$i}/${'country'.$i}", 0700);
