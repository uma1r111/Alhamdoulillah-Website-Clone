...
eQibla.js: Find Qibla Direction (ver 0.2.2)
...
var Qibla = {
	...
	kabaIcon:  {image: 'http://www.al-hamdoulillah.com/images/marker/kaba.png',  width: 17, height: 17},
	homeRIcon: {image: 'http://www.al-hamdoulillah.com/images/marker/rmarker.png', width: 15, height: 20},
	homeLIcon: {image: 'http://www.al-hamdoulillah.com/images/marker/lmarker.png', width: 15, height: 20},
	...
	// default values 
	cookieData: [
		{name: 'homeLat', val: 43.4713},
		{name: 'homeLng', val: -80.5415},
		{name: 'mapZoom', val: 14},
...
Qibla.initMap = function() {
	...
	GEvent.addListener(this.map, 'maptypechanged', function(){ Qibla.redraw(); Qibla.updateListener(); });
	this.updateListener();
}

// update listener
Qibla.updateListener = function() {
	if (this.listener)
		GEvent.removeListener(this.listener);
	if (this.updateMode == 1 || this.getMapType() == 'e') 
		this.listener = GEvent.addListener(this.map, 'click', function(marker, point){ Qibla.setHome(point, marker) });
	else 
		this.listener = GEvent.addListener(this.map, 'move', function(){ Qibla.setHome() });
}
